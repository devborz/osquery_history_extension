...
Option Options::parse(const std::string& option) {
  std::map<std::string, Option> optionsMap{
      std::make_pair("all", all), std::make_pair("filesystem", filesystem),
      std::make_pair("bash", bash), std::make_pair("vim", vim),
...
std::shared_ptr<std::vector<Option>> Options::readOption(
    const bpo::variables_map& vm, unsigned int& ec) {
  std::string option_;
  std::shared_ptr<std::vector<Option>> enabledOptions(
      new std::vector<Option>());
  if (vm.count("help")) {
    Option option = help;
...
void Options::getOptions(bpo::options_description& desc) {
  desc.add_options()("help", "outputs help message")(
      "filesystem", "updates filesystem's history")(
      "bash", "updates bash history's file")(
      "vim", "updates vim history's files")("all",
                                            "updates all history's files");
}
