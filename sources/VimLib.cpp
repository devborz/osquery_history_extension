...
VimCommand VimCommand::parseCommand(const std::string& command,
  ...
  std::string time_s =
      info.substr(time_first_i + 1, time_last_i - time_first_i - 1);
...
VimFileMark::VimFileMark(const std::string& filename, const bfs::path& path,
                         const std::time_t& time, const unsigned int& posRow,
...
VimFileMark VimFileMark::parseFileMark(const std::string& line1,
  ...
  std::string time_s =
      line2.substr(time_first_i + 1, time_last_i - time_first_i - 1);

  std::string path_s =
      line2.substr(path_first_i + 1, path_last_i - path_first_i - 1);

  std::string row_s =
      line2.substr(row_first_i + 1, row_last_i - row_first_i - 1);

  std::string col_s =
      line2.substr(col_first_i + 1, col_last_i - col_first_i - 1);
  ...
  return VimFileMark(path_.filename().string(), path_.parent_path(), time_,
                     row_, col_);
}
